...
#import rospy to be able to make a subscriber to the messages we want to visualize
import rospy
from std_msgs.msg import String

#import the message type for the /vicon/firefly_sbx/firefly_sbx topic
...
class MyViz( QWidget ):
    ...
    def __init__(self):
        ...
        config = rviz.Config()
        reader.readFile( config, "config.pose_truth" )
        ...
        ## it for later.
        print(str(self.manager.getRootDisplayGroup()))
        self.grid_display = self.manager.getRootDisplayGroup().getDisplayAt( 0 )

        #debugging: figure out why this grid is not appearing
        #print(str(self.manager.getRootDisplayGroup().getDisplayAt(0)))
...
#callback function for when a pose message is received
def poseCallback(data,shouldPrint=True):
    """
    Whenever the subscriber sees new data come in, the callback function is called
    set shouldPrint to True to see the topic echoed
    """
    if shouldPrint==True:
        #for now, print out the # of the msg
        print data.header.seq


#function that is called once to start up the listener for pose data
def startPoseListenerNode():
    #create a listener node
    rospy.init_node('viconPoseListener', anonymous=True)

    #create a subscriber to the topic we want
    rospy.Subscriber("/vicon/firefly_sbx/firefly_sbx/", TransformStamped, poseCallback)
...
if __name__ == '__main__':
    ...
    #startPoseListenerNode must be done before everthing else for some reason
    startPoseListenerNode()
    ...
    #rospy.spin() has to go last because it will interfere will the visualization window execution otherwise
    rospy.spin()

    #TODO: figure out why this is preventing me from CTRL-C'ing the program to quit
